...
$MONGO_URI = "mongodb+srv://gauravyt492_db_user:aqGz8IkDZK7jX0RC@osint-bot.imeiuj1.mongodb.net/?appName=Osint-bot";
$MONGO_DB_NAME = "Osint-bot";
...
function getMongoConnection() {
    global $mongoClient, $mongoDb, $MONGO_URI, $MONGO_DB_NAME;
...
function initCollections() {
    ...
    $collections = ['users', 'history', 'user_states', 'payment_stats', 'pending_payments'];
    $existingCollections = [];
    ...
    foreach ($collections as $collection) {
        if (!in_array($collection, $existingCollections)) {
    ...
    try {
        $usersCollection->createIndex(['user_id' => 1], ['unique' => true]);
    } catch (Exception $e) {
    }
    
    $paymentStatsCollection = $db->selectCollection('payment_stats');
    try {
        $paymentStatsCollection->createIndex(['today_date' => 1], ['unique' => true]);
    } catch (Exception $e) {
    }
    
    $pendingPaymentsCollection = $db->selectCollection('pending_payments');
    try {
        $pendingPaymentsCollection->createIndex(['order_id' => 1], ['unique' => true]);
    } catch (Exception $e) {
    }
    
    $existingStats = $paymentStatsCollection->findOne(['today_date' => date('Y-m-d')]);
    if (!$existingStats) {
        $paymentStatsCollection->insertOne([
...
function getUser($userId) {
    ...
    return $collection->findOne(['user_id' => (string)$userId]);
}
...
function createUser($userId, $name, $username, $referredBy = null) {
    ...
    $existingUser = $collection->findOne(['user_id' => $userId]);
        ...
        if ($referredBy) {
            ...
            if ($referrer && $referrer['credits'] !== 'UNLIMITED') {
                ...
                $collection->updateOne(
                    ...
                    ['$set' => ['credits' => (string)($current + $GLOBALS['REFER_BONUS'])]]
                );
            }
        }
    }
...
function updateUserCredits($userId, $creditsToAdd) {
    ...
    if ($user) {
        ...
        $collection->updateOne(
            ['user_id' => $userId],
            ['$set' => ['credits' => (string)($current + $creditsToAdd)]]
        );
        return true;
    }
    return false;
...
function deductCredit($userId) {
    ...
    if ($user) {
        ...
        if (is_numeric($user['credits']) && (int)$user['credits'] > 0) {
            $collection->updateOne(
                ['user_id' => $userId],
                ['$set' => ['credits' => (string)((int)$user['credits'] - 1)]]
            );
            return true;
        }
    }
...
function addHistory($userId, $action, $query, $resultCount) {
    ...
    if ($count > 1000) {
        $excess = $count - 1000;
        $oldRecords = $collection->find([], ['sort' => ['timestamp' => 1], 'limit' => $excess]);
        foreach ($oldRecords as $record) {
            $collection->deleteOne(['_id' => $record['_id']]);
        }
    }
}

function setUserState($userId, $action, $data = "") {
    $collection = getCollection('user_states');
    if ($collection === null) return;
    
    $collection->updateOne(
        ['user_id' => (string)$userId],
        ['$set' => ['action' => $action, 'data' => $data, 'user_id' => (string)$userId]],
        ['upsert' => true]
    );
}
...
function clearUserState($userId) {
    $collection = getCollection('user_states');
    if ($collection === null) return;
    $collection->deleteMany(['user_id' => (string)$userId]);
}

function updateStats($planKey = null, $amount = 0) {
    $collection = getCollection('payment_stats');
    if ($collection === null) return;
    
    $today = date('Y-m-d');
    $stats = $collection->findOne(['today_date' => $today]);
    ...
    if ($planKey && $amount) {
        $update['$inc'] = [
            'today_income' => $amount,
            'total_income' => $amount,
            'plan_counts.' . $planKey => 1
        ];
        $collection->updateOne(['today_date' => $today], $update);
    }
}

function getStats() {
    $collection = getCollection('payment_stats');
    if ($collection === null) return ['today_income' => 0, 'total_income' => 0, 'plan_counts' => [], 'today_date' => date('Y-m-d')];
    
    $today = date('Y-m-d');
    $stats = $collection->findOne(['today_date' => $today]);
    if (!$stats) {
        $stats = ['today_income' => 0, 'total_income' => 0, 'plan_counts' => [], 'today_date' => $today];
    }
    
    $totalIncomeDoc = $collection->aggregate([
        ['$group' => ['_id' => null, 'total' => ['$sum' => '$total_income']]]
    ])->toArray();
    
    $totalIncome = isset($totalIncomeDoc[0]['total']) ? $totalIncomeDoc[0]['total'] : 0;
...
function val($v) {
    return ($v === null || $v === "" || $v === "null" || $v === "N/A" || $v === "None") ? "" : (string)$v;
}
    ...
    if (is_array($data)) {
        ...
        if (is_array($inner)) {
            ...
            foreach ($inner as $k => $v) {
                ...
                if (is_array($v)) {
                    if (has_actual_data(['data' => $v])) return true;
...
function extract_data_for_type($type, $response) {
    ...
    switch ($type) {
        ...
        case 'family':
            return is_array($data) && !isset($data['results']) ? $data : ($data['results'] ?? $data);
        case 'lpg':
        ...
        case 'challan':
            ...
            foreach ($challans as $c) {
                ...
                $offences = [];
                foreach (($c['offece'] ?? []) as $o) {
                    if (is_array($o) && isset($o['offence_name'])) $offences[] = $o['offence_name'];
                }
                $pending[] = [
                    'challan_no' => $c['challan_no'] ?? '',
                    'date' => $c['challan_date'] ?? '',
                    'amount' => $c['challan_amount'] ?? '',
                    'state' => $c['state_code'] ?? '',
                    'status' => $c['challan_status'] ?? '',
                    'violator_name' => $c['violator_name'] ?? '',
                    'offence' => $offences
                ];
            }
            return [
                'reg_no' => $challans[0]['reg_no'] ?? null,
                'owner_name' => $rcInfo['owner_name'] ?? '',
                'vehicle' => $rcInfo['maker_modal'] ?? '',
                'insurance_upto' => '',
...
function validateMobile($d) {
    if (!is_array($d) || count($d) === 0) return false;
    foreach ($d as $e) {
        if (!is_array($e)) continue;
        $m = $e['mobile'] ?? ($e['MOBILE'] ?? ($e['num'] ?? null));
        if ($m && !in_array(trim((string)$m), ['', 'null', 'N/A'])) return true;
    }
    return false;
}

function validateFamily($d) {
    if (!is_array($d) || count($d) === 0) return false;
    $c = 0;
    foreach ($d as $m) {
        if (!is_array($m)) continue;
        $n = $m['memberName'] ?? null;
        if ($n && !in_array(trim((string)$n), ['', 'null', 'N/A', 'None'])) $c++;
    }
    return $c > 0;
}

function validateAadhaar($d) {
    if (is_array($d) && isset($d['id'])) $d = [$d];
    if (!is_array($d) || count($d) === 0) return false;
    foreach ($d as $e) {
        if (!is_array($e) || !isset($e['id']) || in_array(trim((string)$e['id']), ['', 'null', 'N/A']) || !isset($e['name']) || in_array(trim((string)$e['name']), ['', 'null', 'N/A'])) return false;
    }
    return true;
}

function validateVehicle($d) {
    return is_array($d) && isset($d['regNo']) && !in_array(trim((string)$d['regNo']), ['', 'null', 'N/A']) && isset($d['vehicle']) && !in_array(trim((string)$d['vehicle']), ['', 'null', 'N/A']);
}

function validateLpg($d) {
    if (!is_array($d)) return false;
    $c = $d['ConsumerDet'] ?? [];
    return isset($c['ConsumerName']) && !in_array(trim((string)$c['ConsumerName']), ['', 'null', 'N/A']) && isset($c['ConsumerNo']) && !in_array(trim((string)$c['ConsumerNo']), ['', 'null', 'N/A']);
}

function validatePaytm($d) {
    return is_array($d) && isset($d['name']) && !in_array(trim((string)$d['name']), ['', 'null', 'N/A']) && isset($d['phoneNumber']) && !in_array(trim((string)$d['phoneNumber']), ['', 'null', 'N/A']);
}

function validateInstagram($d) {
    if (!is_array($d)) return false;
    $p = $d['profile'] ?? [];
    return isset($p['username']) && !in_array(trim((string)$p['username']), ['', 'null', 'N/A']);
}

function validateFreefire($d) {
    return is_array($d) && isset($d['id']) && !in_array(trim((string)$d['id']), ['', 'null', 'N/A']) && isset($d['nickname']) && !in_array(trim((string)$d['nickname']), ['', 'null', 'N/A']);
}

function validatePan($d) {
    return is_array($d) && isset($d['pan_number']) && !in_array(trim((string)$d['pan_number']), ['', 'null', 'N/A']) && isset($d['name']) && !in_array(trim((string)$d['name']), ['', 'null', 'N/A']);
}

function validatePanToGst($d) {
    if (!is_array($d) || !isset($d['pan']) || in_array(trim((string)$d['pan']), ['', 'null', 'N/A'])) return false;
    $g = $d['gstins'] ?? [];
    if (!is_array($g) || count($g) === 0) return false;
    foreach ($g as $x) {
        if (!is_array($x) || !isset($x['gstin']) || in_array(trim((string)$x['gstin']), ['', 'null', 'N/A'])) return false;
    }
    return true;
}

function validateIfsc($d) {
    return is_array($d) && isset($d['IFSC']) && !in_array(trim((string)$d['IFSC']), ['', 'null', 'N/A']) && isset($d['BANK']) && !in_array(trim((string)$d['BANK']), ['', 'null', 'N/A']);
}

function validatePincode($d) {
    if (!is_array($d) || count($d) === 0) return false;
    $f = $d[0];
    return is_array($f) && isset($f['Pincode']) && !in_array(trim((string)$f['Pincode']), ['', 'null', 'N/A']) && isset($f['Name']) && !in_array(trim((string)$f['Name']), ['', 'null', 'N/A']);
}

function validateTelegram($d) {
    return is_array($d) && isset($d['number']) && !in_array(trim((string)$d['number']), ['', 'null', 'N/A']) && isset($d['tg_id']) && !in_array(trim((string)$d['tg_id']), ['', 'null', 'N/A']);
}

function validateGst($d) {
    return is_array($d) && isset($d['Gstin']) && !in_array(trim((string)$d['Gstin']), ['', 'null', 'N/A']) && isset($d['TradeName']) && !in_array(trim((string)$d['TradeName']), ['', 'null', 'N/A']) && isset($d['LegalName']) && !in_array(trim((string)$d['LegalName']), ['', 'null', 'N/A']);
}

function validateVnum($d) {
    return is_array($d) && isset($d['mobile']) && !in_array(trim((string)$d['mobile']), ['', 'null', 'N/A']) && isset($d['vehicle']) && !in_array(trim((string)$d['vehicle']), ['', 'null', 'N/A']);
}

function validateChallan($d) {
    if (!is_array($d) || !isset($d['reg_no']) || in_array(trim((string)$d['reg_no']), ['', 'null', 'N/A'])) return false;
    $c = $d['pending_challans'] ?? [];
    if (!is_array($c) || count($c) === 0) return false;
    foreach ($c as $x) {
        if (!is_array($x) || !isset($x['challan_no']) || in_array(trim((string)$x['challan_no']), ['', 'null', 'N/A'])) return false;
    }
    return true;
}

function validateIp($d) {
    return is_array($d) && isset($d['query']) && !in_array(trim((string)$d['query']), ['', 'null', 'N/A']) && isset($d['country']) && !in_array(trim((string)$d['country']), ['', 'null', 'N/A']);
}
...
function formatInstagram($d, $c) {
    $p = $d['profile'] ?? [];
    $pic = $p['profile_pic_url_hd'] ?? null;
    $pt = $pic ? "<a href='$pic'>Click Here</a>" : "";
...
function formatPanToGst($d, $c) {
    $g = $d['gstins'] ?? [];
    $msg = "🧾 <b>PAN TO GST INFORMATION</b>\n━━━━━━━━━━━━━━━━━━━━━━━\n💳 <b>PAN NUMBER:</b> <code>" . val($d['pan'] ?? null) . "</code>\n📊 <b>TOTAL GSTINs:</b> <code>" . val($d['total'] ?? null) . "</code>\n━━━━━━━━━━━━━━━━━━━━━━━\n🏢 <b>GSTIN LIST</b>\n━━━━━━━━━━━━━━━━━━━━━━━\n";
    foreach ($g as $i => $x) {
        $st = isset($x['state']) && $x['state'] ? " — 📍 <i>{$x['state']}</i>" : "";
        $msg .= "<b>#" . ($i + 1) . "</b> <code>" . val($x['gstin'] ?? null) . "</code>$st\n";
    }
    $msg .= "━━━━━━━━━━━━━━━━━━━━━━━\n💰 <b>CREDITS REMAINING:</b> <code>$c</code>";
    return $msg;
}

function formatIfsc($d, $c) {
...
function formatChallan($d, $c) {
    $ch = $d['pending_challans'] ?? [];
...
function getForceJoinKeyboard() {
    ...
    return [
        'inline_keyboard' => [
            [['text' => '📢 JOIN CHANNEL', 'url' => "https://t.me/$cu"]],
...
function handleStart($chatId, $userId, $name, $username, $text = '') {
    ...
    if (strpos($text, ' ') !== false) {
        ...
        if (isset($parts[1])) {
            ...
            $collection = getCollection('users');
            if ($collection !== null) {
                $referrer = $collection->findOne(['referral_code' => $refCode]);
                if ($referrer && $referrer['user_id'] !== (string)$userId) {
...
function handleBuy($chatId) {
    ...
    foreach ($CREDIT_PLANS as $pd) {
        $plansText .= "• <b>{$pd['display']}</b>\n";
    }
...
function handleCallback($callbackQuery) {
    ...
    if (strpos($data, 'cancelqr_') === 0) {
        ...
        if ((string)$userId !== $targetUserId && !isAdmin($userId)) {
    ...
    if (strpos($data, 'regen_') === 0) {
        ...
        $keyboard = [
            'inline_keyboard' => [
                [['text' => '✅ VERIFY PAYMENT', 'callback_data' => "verify_{$qrData['order_id']}_{$planKey}_$targetUserId"]],
                [['text' => '❌ CANCEL', 'callback_data' => "cancelqr_$targetUserId"]]
            ]
        ];
    ...
    if (strpos($data, 'verify_') === 0) {
        ...
        if (!$result || ($result['status'] ?? '') !== 'success') {
            ...
            $keyboard = [
                'inline_keyboard' => [
                    ...
                    [['text' => '❌ CANCEL', 'callback_data' => "cancelqr_$planUserId"]]
                ]
            ];
        ...
        $pendingCollection = getCollection('pending_payments');
        $alreadyCredited = false;
        if ($pendingCollection !== null) {
            $existing = $pendingCollection->findOne(['order_id' => $orderId]);
            if ($existing) {
    ...
    if (strpos($data, 'admin_') === 0) {
        switch ($data) {
            case 'admin_stats':
                ...
                $totalUsers = $usersCollection !== null ? $usersCollection->countDocuments() : 0;
                $bannedUsers = 0;
                $unlimitedUsers = 0;
                if ($usersCollection !== null) {
                    $bannedUsers = $usersCollection->countDocuments(['banned' => 1]);
                    $unlimitedUsers = $usersCollection->countDocuments(['credits' => 'UNLIMITED']);
                }
            ...
            case 'admin_view_pending':
                ...
                if ($pendingCollection !== null) {
                    $pending = $pendingCollection->find()->toArray();
                    if (count($pending) > 0) {
            ...
            case 'admin_all_users':
                ...
                $msg = "👥 <b>ALL USERS</b>\n━━━━━━━━━━━━━━━━━━\n";
                if ($usersCollection !== null) {
                    $users = $usersCollection->find([], ['limit' => 50])->toArray();
                    $count = 0;
                    foreach ($users as $u) {
                        $count++;
    ...
    switch ($action) {
        case 'viewhistory':
            ...
            if ($historyCollection !== null) {
                $history = $historyCollection->find(['user_id' => $targetId], ['sort' => ['timestamp' => -1], 'limit' => 20])->toArray();
                if (count($history) > 0) {
                    $count = 0;
        ...
        case 'bulkcredit':
            ...
            if ($amount > 0) {
                ...
                $count = 0;
                if ($usersCollection !== null) {
                    $users = $usersCollection->find(['credits' => ['$ne' => 'UNLIMITED']])->toArray();
                    foreach ($users as $u) {
                        $current = is_numeric($u['credits']) ? (int)$u['credits'] : 0;
                        $usersCollection->updateOne(
                            ['user_id' => $u['user_id']],
                            ['$set' => ['credits' => (string)($current + $amount)]]
                        );
                        $count++;
        ...
        case 'broadcast':
            ...
            sendMessage($chatId, "✅ Broadcast sent to $success/$totalUsers users.");
            clearUserState($userId);
            break;
    }
}
